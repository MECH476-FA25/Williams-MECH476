...
Create a series of EDA plots (cdf, boxplot, histogram, time series) of the `pm2_5_atm` variable from `PA_data_merged`. Use `color =` or `fill =` as an aesthetic to differentiate each sensor by `sensor_ID`. Do the data have a central tendency? Do they appear normally distributed? Do events show up in the time series?  Note: the variable `pm2_5_atm` is the concentration of fine particulate matter air pollution in micrograms per cubic meter ($\mu g/m^3$).
 ...
 Q: Do the data have a central tendency?
 A: Yes. Based on the boxplot and histogram, the data are strongly
    right-skewed, but the central tendency (median, as seen in the
    boxplot) appears to be low, likely between 5 and 10 µg/m³.

 Q: Do they appear normally distributed?
 A: No, not at all. The histogram and CDF clearly show the data is
    heavily right-skewed, with a long tail of high-value outliers.
    A normal distribution would be symmetrical (bell-shaped).

 Q: Do events show up in the time series?
 A: Yes. The time series plot shows a relatively flat baseline with
    several distinct "spikes" or "events" where the PM2.5 concentration
    jumps to a very high level (e.g., > 10 µg/m³) for a short period.
    (Note: With only one file, we only see one sensor's data,
    but the code is set up to show multiple).
